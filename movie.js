...
async function updateUserEmbeddings() {
  try {
    ...
    // return;
    const updates = movies.map(async (movie,index) => {
      const query = "Rohan"
...
// const combineEmbedding = await getEmbeddings(["as"]);
// const combineEmbedding = await getEmbeddings([movie.fullplot]);


// console.log({combineEmbedding})
